...
const Navbar = () => {
  ...
  <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar">
        <div className="w-10 rounded-full">
          <img alt="Tailwind CSS Navbar component" src={userDetails?.photoURL} />
        </div>
      </div>
      ...
      >
        ...
        <span className="relative group-hover:text-[#403F3F]">Sign Out</span>
      </button>
      :
      <Link to={'/login'}>
  <button 
      ...
      >
        <span className="absolute top-0 left-0 flex w-full h-0 mb-0 transition-all duration-200 ease-out transform translate-y-0 bg-gray-50 group-hover:h-full"></span>
        <span className="relative group-hover:text-[#403F3F]">Log In</span>
      </button>
  </Link>
      }
